...
export default function InventoryTransferPage() {
  const [sku,setSku]=useState(""); const [fromWh,setFromWh]=useState(""); const [toWh,setToWh]=useState(""); const [qty,setQty]=useState<number>(1);
  function submit(e:React.FormEvent){e.preventDefault(); alert("Transferred (stub)");}
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <Card className="bg-card dark:bg-[#0f172a] border border-border dark:border-[#1f2937]">
        <CardHeader><CardTitle>Inventory → Transfer</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-12 gap-3">
            ...
            <div className="md:col-span-3"><Label className="text-muted-foreground">From WH</Label>
              <Input value={fromWh} onChange={(e)=>setFromWh(e.target.value)}
                     className="bg-background dark:bg-[#0b1220] border border-input text-foreground" />
            </div>
            <div className="md:col-span-3"><Label className="text-muted-foreground">To WH</Label>
              <Input value={toWh} onChange={(e)=>setToWh(e.target.value)}
                     className="bg-background dark:bg-[#0b1220] border border-input text-foreground" />
            </div>
