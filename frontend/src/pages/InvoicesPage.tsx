...
import { StockAPI, type Invoice, type Paginated } from "../lib/apiStock";
...
export default function InvoicesPage() {
  ...
  async function load() {
    ...
    try {
      const res = await StockAPI.listInvoices(page, pageSize);
      setData(res);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-xl font-semibold">ใบแจ้งหนี้</div>
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              <tr key={idx} className="border-t">
                ...
                <td className="px-3 py-2">{it.inv_no ?? "-"}</td>
...
function PayInvoiceWidget() {
  const [invid, setInvid] = useState("INV-TEST");
  ...
  async function pay() {
    setMsg("");
    try {
      const r = await fetch(`/api/billing/invoices/${encodeURIComponent(invid)}/pay?amount=${encodeURIComponent(amount)}`, {
        method: "POST",
  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      <input value={invid} onChange={e=>setInvid(e.target.value)} className="border rounded px-3 py-1" />
      <input value={amount} onChange={e=>setAmount(e.target.value)} className="border rounded px-3 py-1 w-32 text-right" />
      <button onClick={pay} className="border rounded px-3 py-1">Pay</button>
