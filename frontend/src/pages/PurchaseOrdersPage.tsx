...
import { StockAPI, type PurchaseOrder, type Paginated } from "../lib/apiStock";
...
export default function PurchaseOrdersPage() {
  ...
  async function load() {
    ...
    try {
      const res = await StockAPI.listPurchaseOrders(page, pageSize);
      setData(res);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-xl font-semibold">ใบสั่งซื้อ</div>
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              <tr key={idx} className="border-t">
                ...
                <td className="px-3 py-2">{it.po_no ?? "-"}</td>
                <td className="px-3 py-2">{it.po_date}</td>
...
function POActions() {
  const [poid, setPoid] = useState("PO-TEST");
  const [msg, setMsg] = useState("");

  async function submitPO() {
    setMsg("");
    try {
      const r = await fetch(`/api/purchase-orders/${encodeURIComponent(poid)}/submit`, {
        method: "POST",
  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      <input value={poid} onChange={e=>setPoid(e.target.value)} className="border rounded px-3 py-1" />
      <button onClick={submitPO} className="border rounded px-3 py-1">Submit PO</button>
