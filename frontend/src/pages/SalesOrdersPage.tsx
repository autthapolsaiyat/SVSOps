...
import { StockAPI, type SalesOrder, type Paginated } from "../lib/apiStock";
...
export default function SalesOrdersPage() {
  ...
  async function load() {
    ...
    try {
      const res = await StockAPI.listSalesOrders(page, pageSize);
      setData(res);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-xl font-semibold">ใบสั่งขาย (SO)</div>
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              <tr key={idx} className="border-t">
                ...
                <td className="px-3 py-2">{it.so_no ?? "-"}</td>
                <td className="px-3 py-2">{it.so_date}</td>
...
function SOActions() {
  const [soid, setSoid] = useState("SO-TEST");
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center">
      ...
      <input value={soid} onChange={e=>setSoid(e.target.value)} className="border rounded px-3 py-1" />
      <button onClick={()=>call(`/api/sales/orders/${encodeURIComponent(soid)}/confirm`)} className="border rounded px-3 py-1">Confirm</button>
      <button onClick={()=>call(`/api/sales/orders/${encodeURIComponent(soid)}/deliver`)} className="border rounded px-3 py-1">Deliver</button>
      <button onClick={()=>call(`/api/sales/orders/${encodeURIComponent(soid)}/invoice`)} className="border rounded px-3 py-1">Invoice</button>
