...
// - ตั้งค่า BASE URL จาก VITE_API_URL หรือ VITE_API_BASE (dev: http://localhost:8080/api)
...
export async function logoutApi(): Promise<{ ok: boolean }> {
  // เรียก backend ให้ออกระบบ (ถ้ามี), แล้วเคลียร์ฝั่ง client เสมอ
  try {
  ...
  try {
    localStorage.removeItem("access_token");
    localStorage.removeItem("token");        // ✅ ครอบคลุมทั้งสอง key
    localStorage.removeItem("auth_user");
    localStorage.removeItem("auth_perms");
  } catch {}
  window.dispatchEvent(new CustomEvent("auth:401"));
...
export async function permsList(): Promise<string[]> {
  try {
    ...
    // fallback demo labels (กรณี backend ไม่รองรับ endpoint นี้)
    return [
      "ดูบทบาท",
      "ดูบันทึกกิจกรรม",
      "ดูผู้ใช้",
