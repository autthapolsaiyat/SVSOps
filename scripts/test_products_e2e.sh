...
if [ "$ME_STATUS" != "200" ]; then
  echo "ERROR: login failed (ลองตั้งค่า ACC/PWD ให้ถูกต้อง เช่น ACC=admin PWD=admin)"
  exit 1
fi
...
# 7) Lookups + ตัวอย่างฟิลเตอร์ origin
