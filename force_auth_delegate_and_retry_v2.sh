...
# (b) แทรก resolver ที่เรียก /api/auth/me ภายใน
...
'''
    s = s + inject  # ต่อท้ายไฟล์แบบตรงๆ ป้องกันหา anchor ไม่เจอ
...
# 3) requirements: แก้ import bug (ใช้ re ให้ถูก) + เติม httpx/passlib
python3 - <<'PY'
from pathlib import Path
import re as _re
...
if _re.search(r'^passlib', txt, flags=_re.M):
    new = _re.sub(r'^passlib.*$', 'passlib[bcrypt]==1.7.4', txt, flags=_re.M)
    if new != txt:
