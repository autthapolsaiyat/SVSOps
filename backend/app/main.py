...
async def _current_user_dep(request: Request):
    try:
        ...
        if gcu is None:
            raise RuntimeError("get_current_user not found in app.routers.auth")
        if inspect.iscoroutinefunction(gcu):
            return await gcu(request)   # type: ignore
        return gcu(request)             # type: ignore
...
async def products_upsert(body: UpsertProductBody, user=Depends(_current_user_dep)):
...
async def products_toggle_active(body: ToggleBody, user=Depends(_current_user_dep)):
    if not (_roles_of(user) & {"admin","superadmin"}):
        raise HTTPException(status_code=403, detail="forbidden")
...
async def stock_log_receive(body: LogRecvBody, user=Depends(_current_user_dep)):
...
async def stock_log_issue(body: LogIssueBody, user=Depends(_current_user_dep)):
    if not (_roles_of(user) & {"stock","admin","superadmin"}):
        raise HTTPException(status_code=403, detail="forbidden")
