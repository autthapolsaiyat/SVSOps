...
async def _ensure_tables():
    ...
    ddls = [
        ...
        CREATE TABLE IF NOT EXISTS customers_meta (
          code        TEXT PRIMARY KEY REFERENCES customers(code) ON DELETE CASCADE,
          team_code   TEXT,
          group_code  TEXT,
          group_name  TEXT,
...
# ---------- Schemas ----------
class UpsertCustomerBody(BaseModel):
    code: str
    name: str
...
async def customers_upsert(body: UpsertCustomerBody, user=Depends(_resolve_current_user)):
...
@router.post("/customers/active")
async def customers_toggle_active(body: ToggleBody, user=Depends(_resolve_current_user)):
    if not (_roles_of(user) & {"admin", "superadmin"}):
        raise HTTPException(status_code=403, detail="forbidden")
    await _ensure_tables()
    async with _engine.begin() as conn:
        # ensure main exists
        await conn.execute(sa.text("""
            INSERT INTO customers (code, name) VALUES (:code, :name)
            ON CONFLICT (code) DO NOTHING
        """), {"code": body.code, "name": body.code})
        # update meta
        await conn.execute(sa.text("""
            INSERT INTO customers_meta (code, is_active) VALUES (:code, :v)
